...
# Pasta de dados:
# Localmente: ./data
# No Railway: /app/data quando o Volume for montado em /app/data
DATA_DIR = Path(os.environ.get("DATA_DIR", BASE / "data"))

DB = DATA_DIR / "mobiles.db"
...
def init_db():
    with get_db() as db:
        ...
        CREATE INDEX IF NOT EXISTS idx_mobiles_numero
        ON mobiles(numero);

        CREATE INDEX IF NOT EXISTS idx_mobiles_entrega
        ON mobiles(entrega_id);
        """)
...
def criar_entrega():
    ...
    try:
        numeros = [
            normalize(x)
            for x in json.loads(request.form.get("numeros", "[]"))
        ]
    except Exception:
        numeros = []
    ...
    if (
        quantidade < 1
        or quantidade > 50
        or len(numeros) != quantidade
        or any(not x for x in numeros)
    ):
        return jsonify(
            error="Informe corretamente a quantidade e os números dos mobiles."
        ), 400

    if len(set(numeros)) != len(numeros):
        return jsonify(
            error="Não repita o mesmo número de mobile nesta entrega."
        ), 400

    if not arquivo or not arquivo.filename:
        return jsonify(
            error="Selecione a foto/scan do documento."
        ), 400
